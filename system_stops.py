...
async def _drain_bp_propagation(client, target_id):
    ...
    try:
        ...
        # #1 capture-mode re-arm НЕ здесь (live-fix 2026-06-03): setBreakOnNext-
        # Statement глобален («следующая инструкция на ЛЮБОМ таргете»), поэтому
        # re-arm сразу после Continue заставлял ТЕКУЩИЙ таргет single-step'иться
        # по всем строкам → JOB падал «ЗавершеноСОшибкой» (deep-тест). Re-arm для
        # СЛЕДУЮЩЕГО нового таргета теперь на targetQuit (см. _handle_command).
    except Exception as e:
